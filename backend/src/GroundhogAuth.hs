...
initGroundhogAuth sess pool = makeSnaplet "groundhog-auth" desc datadir $ do
    ...
    c <- liftIO $ extractConn return pool
    let migrateDB = runMigration $ migrate (undefined :: AuthUser)
    liftIO $ runNoLoggingT ((runDbPersist migrateDB) c)
...
runGH
    ...
    -> m a
runGH pool action = liftIO (extractConn return pool) >>=
                    runNoLoggingT . withConn (runDbPersist action)
